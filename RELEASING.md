...
5. Create an annotated tag on current remote `main`, for example `git tag -a v0.2.1 -m "v0.2.1"` (or `-s` when signing is configured), then push only that tag.
...
```bash
gh release download v0.2.1 -R ryanduguid/xero-ledger-review-gate --dir release-v0.2.1
cd release-v0.2.1
sha256sum --check SHA256SUMS
gh attestation verify elizabeth_anne_alexander-0.2.1-py3-none-any.whl -R ryanduguid/xero-ledger-review-gate
gh attestation verify elizabeth_anne_alexander-0.2.1-py3-none-any.whl -R ryanduguid/xero-ledger-review-gate --predicate-type https://spdx.dev/Document/v2.3
gh release view v0.2.1 -R ryanduguid/xero-ledger-review-gate --json isImmutable
gh release verify v0.2.1 -R ryanduguid/xero-ledger-review-gate
gh release verify-asset v0.2.1 elizabeth_anne_alexander-0.2.1-py3-none-any.whl -R ryanduguid/xero-ledger-review-gate
```
